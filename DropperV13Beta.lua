...
local function broadcastControlMode(mode: string)
	...
	pcall(function()
		...
		if channels then
			target = channels:FindFirstChild("RBXGeneral") or channels:FindFirstChildOfClass("TextChannel")
		end
		if target == nil then
			target = tcs:FindFirstChildOfClass("TextChannel", true)
		end
		if target ~= nil then
			(target :: any):SendAsync(CONTROL_CMD_PREFIX .. mode)
		end
	end)
	print("[Dropper] Control mode -> " .. mode)
end
...
local function createHostUI()
	...
	local hideClientToggle = makeToggleRow("Hide Client Name", "Hide only the client name in leaderboard", isHideClientNameOn(), 3, function(on)
